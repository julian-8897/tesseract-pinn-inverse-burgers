...
The observations are synthetic but honest: they come from a differentiable
pseudospectral Burgers solver (FFT spatial derivatives, 2/3 dealiasing, adaptive
Diffrax time integration), sampled at sparse space–time points with additive Gaussian
noise ($\sigma = 0.02$ by default). Because the data is generated by the real nonlinear
PDE rather than an analytic approximation, the inverse problem is genuine — and the
solver-adjoint method below gives a ground-truth-quality reference to measure the
others against.
...
Because every method speaks that same interface, switching between them — a JAX
component for a PyTorch one, or a point estimate for a full posterior — is a one-line
change that leaves the optimization loop untouched:
...
That swappability is the point. One `jax.grad`-based outer loop drives a JAX solver, a
JAX *or* PyTorch PINN, and a flow-matching posterior net, because each is a versioned,
framework-agnostic component behind the same contract.
...
Unlike the two deterministic methods, this one is *amortized*: the cost is paid once,
offline, and every new observation is then a single forward pass.
...
<p align="center">
  ...
  narrow — a mild overconfidence the simulation-based calibration (SBC) below measures
  rather than hides.</em>
</p>
