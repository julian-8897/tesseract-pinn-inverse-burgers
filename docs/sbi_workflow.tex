...
% Shared brand palette (refined light): teal -> indigo -> crimson progression
\definecolor{cSim}{HTML}{0E6E6E}    % deep teal (simulate, uses solver)
\definecolor{cTrain}{HTML}{3D3D9E}  % indigo (train flow)
\definecolor{cInfer}{HTML}{9E2B3A}  % crimson (inference)
\definecolor{cStage}{HTML}{44566C}  % slate
...
\begin{tikzpicture}[
  ...
  stage/.style={rounded corners=4pt, line width=0.9pt, align=center,
...
\node[stage, fill=cSim!15, draw=cSim] (s1) at (0,0) {};
...
\node[pill, draw=cSim!55, fill=cSim!18] (solv) at (0,-0.35)
...
\node[stage, fill=cInfer!15, draw=cInfer] (s3) at (14.8,0) {};
...
\node[pill, draw=cInfer!55, fill=cInfer!18] (post) at (14.8,-0.35)
