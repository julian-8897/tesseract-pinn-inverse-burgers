...
\begin{tikzpicture}[
  ...
  stage/.style={rounded corners=4pt, line width=0.7pt, align=center,
                minimum width=60mm, minimum height=34mm, inner sep=7pt},
...
\node[pill, draw=cSim!55] (prior) at (0,0.55)
  {prior \ $\bm{\theta}=(\nu, A_{\mathrm{IC}}, \varphi_{\mathrm{IC}})$};
\node[pill, draw=cSim!55, fill=cSim!10] (solv) at (0,-0.45)
  {\texttt{burgers\_solver}};
\node[pill, draw=cSim!55] (xobs) at (0,-1.45)
...
\node[pill, draw=cTrain!55] (pairs) at (7.4,0.55)
  {pairs $(\bm{\theta},\bm{x})$};
\node[pill, draw=cTrain!55, fill=cTrain!10] (fmpe) at (7.4,-0.45)
  {FMPE \ (flow matching, \texttt{zuko})};
\node[pill, draw=cTrain!55] (qnet) at (7.4,-1.45)
...
\node[pill, draw=cInfer!55] (newx) at (14.8,0.55)
  {new observation $\bm{x}$};
\node[pill, draw=cInfer!55, fill=cInfer!8] (post) at (14.8,-0.45)
  {posterior samples $\bm{\theta}\sim q_\phi$};
\node[pill, draw=cInfer!55] (diag) at (14.8,-1.45)
  {marginals $+$ SBC / TARP};
...
% banner
\node[note, text=black!60, anchor=south] at (7.4,2.35)
