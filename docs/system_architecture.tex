...
\definecolor{cLoop}{HTML}{37474F}    % outer loop (dark slate)
\definecolor{cBar}{HTML}{546E7A}     % contract bar
\definecolor{cSolver}{HTML}{1F77B4}  % solver (blue)
\definecolor{cPinn}{HTML}{2CA02C}    % PINN (green)
\definecolor{cFmpe}{HTML}{6A51A3}    % posterior (purple)
...
\begin{tikzpicture}[
  ...
  comp/.style={rounded corners=3pt, draw=black!45, line width=0.6pt,
               align=center, minimum width=52mm, minimum height=21mm,
               inner sep=5pt},
  applyarr/.style={->, draw=black!45, line width=0.8pt},
  vjparr/.style={->, draw=cPinn!75!black, line width=0.8pt},
...
\node[rounded corners=3pt, fill=cBar!16, draw=cBar, text=cBar!45!black,
...
\node[comp, fill=cSolver!10, draw=cSolver, text=cSolver!55!black] (solver) at (3,0.2)
...
\node[comp, fill=cPinn!10, draw=cPinn, text=cPinn!45!black] (pinn) at (9,0.2)
  {\textbf{PINN}\\[2pt]
...
\node[comp, fill=cFmpe!10, draw=cFmpe, text=cFmpe!50!black] (fmpe) at (15,0.2)
...
% small legend for the two arrow directions
\node[tag, anchor=west] at (0.4,1.65) {$\downarrow$ \texttt{apply}};
\node[tag, anchor=west, text=cPinn!70!black] at (0.4,1.25) {$\uparrow$ VJP (gradient)};
