% System / contract diagram: one outer loop, a shared Tesseract interface
% pattern, and three framework-agnostic component roles. Only the two PINN
% implementations are literal schema-compatible swaps.
...
\node[rounded corners=3pt, fill=cLoop, text=white, align=center,
  ...
  {\textbf{outer optimization loop}\\[1pt]
   \footnotesize\texttt{jax.value\_and\_grad} / \texttt{optax} over $\log\nu$\\[-1pt]
   \footnotesize plus PINN parameters where applicable};
...
\node[rounded corners=3pt, fill=cBar!20, draw=cBar, text=cBar!45!black,
      ...
      (bar) at (9,3.4)
  {\textbf{Tesseract component boundary}\ \ --- \ \ component-specific typed
   \texttt{apply} contracts\\[-1pt]
   $+$ \ \texttt{vector\_jacobian\_product} where gradients are required};

\draw[applyarr, draw=cLoop, line width=1pt] (loop) -- (bar)
  node[midway, right=1pt, tag, text=cLoop] {drives the gradient-based methods};
...
\node[comp, fill=cSolver!18, draw=cSolver, text=cSolver!55!black] (solver) at (3,0.2)
  {\textbf{burgers\_solver}\\[2pt]
   ...
   \footnotesize VJP $=$ discrete solver adjoint};
...
\node[comp, fill=cFmpe!18, draw=cFmpe, text=cFmpe!50!black] (fmpe) at (15,0.2)
  {\textbf{fmpe\_posterior}\\[2pt]
   ...
   \footnotesize \emph{trained offline}, apply-only\\[-1pt]
...
\node[font=\sffamily\footnotesize\itshape, text=black!60, align=center]
  (cap) at (9,-1.9)
  {Each versioned container defines its own typed contract and follows the same
   Tesseract \texttt{apply}/VJP pattern.\\ Only the two PINN backends share an identical
   schema: swapping them is a single \texttt{Tesseract.from\_image(...)} call.};
