...
\node[font=\sffamily\footnotesize\itshape, text=black!60, align=center]
  (cap) at (9,-1.9)
  {Each component is a versioned, framework-agnostic container sharing the
   \texttt{apply}/VJP pattern.\\ The two PINN backends share one schema, so swapping them
   is a single \texttt{Tesseract.from\_image(...)}; the solver and posterior keep their
   own contracts.};
